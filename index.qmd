...
ax.axhline(mean_dreamy, color=dreamy_color, linestyle='--', linewidth=1.4, alpha=0.9, zorder=2)
ax.axhline(mean_duck, color=duck_color, linestyle='--', linewidth=1.4, alpha=0.9, zorder=2)
...
ax.set_xlim(0, patients_df['patient'].max() + 1)
...
dreamy = patients_randomized_df[patients_randomized_df['doctor_name'] == 'Doc Dreamy']
duck = patients_randomized_df[patients_randomized_df['doctor_name'] == 'Doc Duck']
mean_dreamy = dreamy['post_surgical_score'].mean()
mean_duck = duck['post_surgical_score'].mean()
...
ax.set_title('Post-Surgical Symptom Score (Randomized Assignment)', fontsize=13, fontweight='semibold')
ax.set_xlabel('Patient Number', fontsize=11)
ax.set_ylabel('Post-Surgical Symptom Score (Lower is Better)', fontsize=11)
ax.grid(True, linestyle=':', alpha=0.45)
ax.legend(loc='upper left', framealpha=0.95)
ax.set_xlim(0, patients_randomized_df['patient'].max() + 1)
...
### Figure 3: Outcomes by Patient Severity
...
ax.axvspan(overlap_xmin, overlap_xmax, alpha=0.15, color='gray', zorder=0)
...
## Solving the Confounder Challenge
